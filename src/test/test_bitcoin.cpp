...
#define BOOST_TEST_MODULE Qcity Test Suite
...
TestingSetup::TestingSetup(const std::string& chainName) : BasicTestingSetup(chainName)
{
    const CChainParams& chainparams = Params();
        ...
        ClearDatadirCache();
        pathTemp = GetTempPath() / strprintf("test_qcity_%lu_%i", (unsigned long)GetTime(), (int)(GetRand(100000)));
...
TestChain100Setup::TestChain100Setup() : TestingSetup(CBaseChainParams::REGTEST)
{
    ...
    for (int i = 0; i < Params().GetConsensus().COINBASE_MATURITY; i++)
    {
