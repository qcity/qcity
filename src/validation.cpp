...
#if defined(NDEBUG)
# error "Qcity cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Qcity Signed Message:\n";
using namespace std;
// Internal stuff
namespace {
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s QCT additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(const CBlockIndex * pindexPrev , const Consensus::Params& consensusParams)
{
    ...
    if(pindexPrev==NULL ) { 
        ...
        nSubsidy = BLOCK_REWARD_COIN_100; // reward change 0.06
    }
    //limit of reward
    ...
    if (!IsInitialBlockDownload())
    {
        ...
        for (int i = 0; i < 100 && pindex != NULL; i++)
        {
            ...
            if (pindex->nVersion > VERSIONBITS_LAST_OLD_BLOCK_VERSION && (pindex->nVersion & ~nExpectedVersion) != 0) { 
                if(! (pindex->nVersion & VERSION_BLOCK_SIG ))//remove warning
                    ++nUpgraded;
            }
            pindex = pindex->pprev;
        }
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Check proof of work
    if (!block.IsProofOfOnline() && block.nBits != GetNextWorkRequired(pindexPrev, &block, consensusParams  )) { 
        //DbgMsg(" block.nBits %d, require:%d" ,block.nBits , GetNextWorkRequired(pindexPrev, &block, consensusParams ));
